...
export default function AdminContainersScreen() {
  const [items, setItems] = useState([]);
  const [formData, setFormData] = useState({
    code: "",
    name: "",
    container_type: "SCRAP",
    capacity: 1000,
    unit: "KG",
    area_id: 2,
  });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axiosClient.post("/scrap/containers", formData);
      fetchItems();
      setFormData({ ...formData, code: "", name: "", capacity: 1000 });
    } catch (error) {
      console.error(error);
    }
  };
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">Administración: Contenedores Scrap/Merma</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-surface p-4 rounded-lg shadow mb-6 flex flex-wrap gap-4">
        ...
        <select className="border p-2 rounded" value={formData.container_type} onChange={(e) => setFormData({...formData, container_type: e.target.value})}>
        ...
        <tbody>
          {filteredItems.map((item) => (
            <tr key={item.id} className="border-b">
              ...
              <td className="p-3">{item.container_type}</td>
