...
export default function AdminSpoolsScreen() {
  const [items, setItems] = useState([]);
  const [formData, setFormData] = useState({
    code: "",
    name: "",
    material_family: "HILO",
    color: "",
    caliber_value: "",
    caliber_unit: "TEX",
    presentation: "CARRETE",
    primary_unit: "CARRETE",
    production_area_id: 2, // Extrusion
  });
  ...
  const fetchItems = async () => {
    try {
      ...
      setItems(extractArray(response).filter((i) => i.presentation === "CARRETE"));
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fetchItems();
      setFormData({ ...formData, code: "", name: "", color: "", caliber_value: "" });
    } catch (error) {
      console.error(error);
    }
  };
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">Administración: Tipos de Carretes</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-surface p-4 rounded-lg shadow mb-6 flex flex-wrap gap-4">
        ...
        <input placeholder="Color" className="border p-2 rounded" value={formData.color} onChange={(e) => setFormData({...formData, color: e.target.value})} />
        <input placeholder="Calibre" type="number" className="border p-2 rounded" value={formData.caliber_value} onChange={(e) => setFormData({...formData, caliber_value: e.target.value})} />
        ...
        <tbody>
          {filteredItems.map((item) => (
            <tr key={item.id} className="border-b">
              ...
              <td className="p-3">{item.caliber_value} {item.caliber_unit}</td>
